...
class ExportController extends Controller
{
	...
	{
		return view('tenant.invoice-lines.export');
	}
	...
	public function run(UpdateExportRequest $request, Export $export)
	{
		...
		switch ($export->entity) {
			...
			case EntityEnum::INVOICE->value:
				...
				break;
			case EntityEnum::INVOICELINE->value:
					return self::exportInvoiceLine($start_date, $end_date, $currency, $dept_id, $supplier_id, $project_id, $warehouse_id, $bank_account_id, $user_id);
				break;
	...
	public function exportInvoiceLine($start_date, $end_date, $currency, $dept_id, $supplier_id, $project_id, $warehouse_id, $bank_account_id, $user_id)
	{

		$this->authorize('invoiceLine', Export::class);
		...
		// Filter based on input
		$invoiceLines->whereHas('invoice', function ($q) use ($start_date, $end_date ) {
			$q->whereBetween('invoice_date', [$start_date, $end_date ]);
		});

		if ($currency <> ''){
			$invoiceLines->whereHas('invoice', function ($q) use ($currency) {
				$q->where('currency', $currency);
			});
		}

		if ($dept_id <> ''){
			$invoiceLines->whereHas('invoice', function ($q) use ($dept_id) {
				$q->where('dept_id', $dept_id);
			});
		}

		if ($supplier_id <> ''){
			$invoiceLines->whereHas('invoice', function ($q) use ($supplier_id) {
				$q->where('supplier_id',$supplier_id);
			});
		}

		if ($project_id <> ''){
			$invoiceLines->whereHas('invoice', function ($q) use ($project_id) {
				$q->where('project_id',$project_id);
			});
		}
		...
		if (auth()->user()->role->value == UserRoleEnum::HOD->value){
			$invoiceLines->whereHas('invoice', function ($q) {
				$q->where('dept_id', auth()->user()->dept_id);
			});
		}
