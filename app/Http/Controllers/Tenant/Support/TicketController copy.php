...
class TicketController extends Controller
{
	...
	public function store(StoreTicketRequest $request)
	{
		...
		$name='User 2 by Tenant';
		$email='user2@example.com';

		$landlordUser = tenancy()->central(function ($tenant)  use ($name,$email) {
						Log::debug("id= ".$email);
		...
		$aa = tenancy()->central(function ($tenant)  use ($email, $id) {
			Log::debug("id= ".$email);
			$user = \App\Models\User::where('email', $email)->first();
			if(is_null($user)) {
				Log::debug("first not found TODO in landlord user create user= ".$email);   
				$user = \App\Models\User::create([
			...
			try {
				$account = \App\Models\User::where('email', $email)->firstOrFail();
				Log::debug("firstOrFail found = ".$email);
			} catch (ModelNotFoundException $exception) {
				Log::debug("firstOrFail not found TODO in landlord user create user= ".$email);
		...
		return redirect()->route('dashboards.index')->with('success', 'Ticket #' . $ticket->id . ' created successfully.');
	}
