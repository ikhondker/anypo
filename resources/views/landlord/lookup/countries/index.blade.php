...
@section('content')
	...
	<div class="card">
		...
		<div class="card-header d-sm-flex justify-content-sm-between align-items-sm-center border-bottom">
			<h5 class="card-header-title">Country List</h5>
			<a class="btn btn-primary btn-sm" href="{{ route('countries.create') }}">
				<i class="bi bi-plus-square me-1"></i> Create Country
			</a>
		</div>
						...
						<tr>
							<td>
								<div class="d-flex align-items-center">
									...
									<div class="flex-grow-1 ms-3">
										<a class="d-inline-block link-dark" href="{{ route('countries.show', $country->country) }}">
											<h6 class="text-hover-primary mb-0">
												{{ $country->name }}
											</h6>
										</a>
										<small class="d-block">{{ $country->name }}</small>
									</div>
								</div>
							</td>
							...
							<td>
								...
								<a href="{{ route('countries.delete', $country->country) }}"
									...
									<i class="bi {{ $country->enable ? 'bi-bell-slash' : 'bi-bell' }} "
