...
@section('content')
	...
	<div class="d-grid gap-1 gap-lg-1">
		<div class="row">
			<x-landlord.widget.kpi value="{{ $count_tickets }}" route="tickets"  label="ALL TICKETS" icon="com013"/>
	...
	<div class="d-grid gap-1 gap-lg-1">
		<div class="row">
			<x-landlord.widget.kpi value="{{ $count_accounts }}" label="TOTAL ACCOUNTS" icon="com013" route="accounts"/>
			<x-landlord.widget.kpi value="{{ $count_service }}" label="TOTAL SERVICES" icon="abs029" route="services"/>
			<x-landlord.widget.kpi value="{{ $count_invoices }}" label="TOTAL INVOICES"  icon="abs027" route="invoices"/>
			<x-landlord.widget.kpi value="{{ $count_payments }}" label="TOTAL PAYMENTS" icon="com006" route="payments"/>
		</div>
		<!-- End Row -->
	</div>
