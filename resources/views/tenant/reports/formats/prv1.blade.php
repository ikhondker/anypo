...
<body>
	...
	<table id="my-header-table">
		...
		<tr>
			...
			<td style="text-align:right; vertical-align: bottom;">
				...
				REQUISITION <strong>#{{ $pr->id}}</strong><br>
				DATE: {{ date('d-M-Y') }} <br>
	...
	<table id="my-summary-table">
		...
		<tr>
			<td style="width:40%; text-align:left;"><strong>REQUESTOR</strong></td>
			<td style="width:20%; text-align:left"></td>
			<td style="width:40%; text-align:right;"><strong>PROPOSED VENDOR</strong></td>
		</tr>
		<tr>
	...
	<table id="my-table">
		...
		<tr>
			...
			<td>{{ $prl->summary }}</td>
			<td>{{ $prl->item->uom->name }}</td>
			<td style="text-align:right;">{{ number_format($prl->qty,2) }}</td>
			<td style="text-align:right;">{{ number_format($prl->price,2) }}</td>
			<td style="text-align:right;">{{ number_format($prl->sub_total,2) }} </td>
		</tr>
		@endforeach
	   
		<tr>
			<th width="15%" colspan="5" style="text-align:right">SUB TOTAL</th>
			<th width="15%" style="text-align:right">{{ number_format($pr->sub_total,2) }}</th>
		</tr>
	</table>
			...
			<td style="vertical-align: top;">
				<table id="my-total-table">
					<tr>
						...
						<td width="15%" style="text-align:right">{{ number_format($pr->tax,2) }}</td>
					</tr>
					<tr>
						<td style="text-align:right">Shipping:</td>
						<td style="text-align:right">{{ number_format($pr->shipping,2) }}</td>
					</tr>
					<tr>
						<td style="text-align:right">Discount:</td>
						<td style="text-align:right">{{ number_format($pr->discount,2) }}</td>
					</tr>
					<tr>
						<td style="text-align:right; background-color: #1F9BCF;"><h3 style="color:#FFFFFF">TOTAL {{ $pr->currency }}<h3></td>
						<td style="text-align:right; background-color: #1F9BCF;"><h3 style="color:#FFFFFF">{{ number_format($pr->amount,2) }}</h3></td>
					</tr>
				</table>
			</td>
		</tr>
